...
export default function CoursesPage() {
  ...
  return (
        ...
        <button
          ...
          className={`text-xs px-3 py-1.5 rounded-lg border transition-all font-mono ${
            activeOnly
              ...
              : "bg-ink-700 border-ink-600 text-slate-400 hover:border-slate-400"
          }`}
        >
          {activeOnly ? "● Active Only" : "○ All Courses"}
        </button>
        <span className="text-slate-500 text-xs font-mono">{courses.length} course{courses.length !== 1 ? "s" : ""}</span>
      </div>
    ...
    <tr><td colSpan={6}><EmptyState          icon="◎"          title="No courses found"          description={activeOnly ? "Try unchecking 'Active only' to see all courses." : "Create your first course to get started."}
