...
const ROLE_COLORS = {
  ADMIN:    { dot: "bg-accent-rose",  text: "text-accent-rose"  },
  MANAGER:  { dot: "bg-accent-amber", text: "text-accent-amber" },
  EMPLOYEE: { dot: "bg-brand",        text: "text-brand"        },
};
      ...
      <aside className={`flex flex-col border-r border-ink-700 bg-ink-900 transition-all duration-300 ${collapsed ? "w-16" : "w-60"}`}>
        ...
        {!collapsed && role && (
          <div className="mx-3 mt-3 px-3 py-2 rounded-lg bg-ink-800 border border-ink-600 flex items-center gap-2">
            ...
            <span className={`text-xs font-mono font-semibold ${roleColor.text}`}>{role}</span>
          </div>
        )}
